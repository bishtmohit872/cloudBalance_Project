    ...
    return(
        <div className={`h-full ${state?'w-[304px]':'w-[90px]'} relative p-4 transition-all duration-400 bg-white shadow-[4px_0_6px_rgba(0,0,0,0.15)]`}>
                ...
                <div className="h-max py-4">
                    {
                        MenuList.map((menu)=>(
                            <NavLink key={menu.id} to={menu.path} className={({isActive})=>`${isActive ? " bg-[#499FDC] rounded-md text-white shadow-lg":""} mb-4 p-4 w-full flex items-center justify-start text-blue-950 overflow-hidden hover:bg-blue-400 hover:text-white rounded-lg`}>
                                <span>{menu.logo}</span>
